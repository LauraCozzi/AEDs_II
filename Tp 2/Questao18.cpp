...
bool buscaBinaria(Lista *lista, char* nome);
void ordenarPeso(Lista *lista);
void constroi(int tamHeap, Lista *lista);
void reconstroi(int tamHeap, Lista* lista);
void radixsort(Lista* lista, int tamanho);
void swap(int i, int j, Lista *lista);
...
void radixsort(Lista* lista, int tamanho) {
    int i;
    Lista *b;
    int maior = lista->array[0]->altura;
    int exp = 1;

    b = (Lista*)malloc(sizeof(Lista));

    for (i = 0; i < tamanho; i++) {
        if (lista->array[i]->altura > maior)
            maior = lista->array[i]->altura;
    }

    while (maior/exp > 0) {
        int bucket[10] = { 0 };
        for (i = 0; i < tamanho; i++)
            bucket[(lista->array[i]->altura / exp) % 10]++;
        for (i = 1; i < 10; i++)
            bucket[i] += bucket[i - 1];
        for (i = tamanho - 1; i >= 0; i--)
            b->array[--bucket[(lista->array[i]->altura / exp) % 10]] = lista->array[i];
        for (i = 0; i < tamanho; i++)
            lista->array[i] = b->array[i];
        exp *= 10;
    }
...
void ordenarPeso(Lista *lista){
        ...
        for(int i = 1; i < lista->n; i++){
            ...
            int j = i - 1;
            while(j >= 0 && strcmp(lista->array[j]->nome, tmp->nome) > 0 && lista->array[j]->altura == tmp->altura){
                    lista->array[j+1] = clone(lista->array[j]);
                    j--;
                
            }
            lista->array[j+1] = clone(tmp);
    }
}
