...
void ordenarAnoNascimento(Lista *lista);
   ...
   for (int i = (lista->n - 1); i > 0; i--) {
      for (int j = 0; j < i; j++) {
         if (strcmp(lista->array[j]->anoNascimento,lista->array[j + 1]->anoNascimento) > 0) {
            Personagem* auxiliar = lista->array[j + 1];
            lista->array[j + 1] = lista->array[j];
            lista->array[j] = auxiliar;
         }
      }
    }
}
...
void ordenarAnoNascimento(Lista *lista){
        ...
        for(int i = 1; i < lista->n; i++){
            ...
            int j = i - 1;
            while(j >= 0 && strcmp(lista->array[j]->nome, tmp->nome) > 0 && strcmp(lista->array[j]->anoNascimento,tmp->anoNascimento) == 0){
