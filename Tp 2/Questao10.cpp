...
void SelectionSortR(Lista *lista, int i, int n);
void swap(int i, int j, Lista *lista);
int selecao(int i, Lista* lista, int menor, int n);
...
int main(){
    ...
    SelectionSortR(lista, 0, lista->n);
    mostrar(lista);
...
int selecao(int i, Lista* lista, int menor, int n){
    ...
    for(int j = i+1; j < n; j++){
        if(strcmp(lista->array[menor]->nome,lista->array[j]->nome) > 0)
            menor = j;
    }
