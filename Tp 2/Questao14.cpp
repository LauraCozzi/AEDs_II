...
void quicksortRec (int esq, int dir, Lista *lista);
void ordenarCorCabelo(Lista *lista);
...
void ordenarCorCabelo(Lista *lista){
        ...
        for(int i = 1; i < lista->n; i++){
            ...
            int j = i - 1;
            while(j >= 0 && strcmp(lista->array[j]->nome, tmp->nome) > 0 && strcmp(lista->array[j]->corDoCabelo, tmp->corDoCabelo) == 0){
...
void quicksortRec (int esq, int dir, Lista *lista) {
   ...
   while (i <= j) {
      while (strcmp(lista->array[i]->corDoCabelo, pivo->corDoCabelo) < 0) i++;
      while (strcmp(lista->array[j]->corDoCabelo, pivo->corDoCabelo) > 0) j--;
      if (i <= j) {
         swap(i, j, lista);
         i++;
         j--;
...
void quicksort(Lista *lista) {
   quicksortRec(0, lista->n-1, lista);
}
