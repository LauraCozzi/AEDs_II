...
void constroi(int tamHeap, Lista *lista){
    for(int i = tamHeap; i > 1 && lista->array[i]->altura > lista->array[i/2]->altura; i /= 2){
        swap(i, i/2, lista);
...
void reconstroi(int tamHeap, Lista* lista){
    ...
    while(i <= (tamHeap/2)){
        ...
        if (lista->array[2*i]->altura > lista->array[2*i+1]->altura || 2*i == tamHeap){
            filho = 2*i;
        } else {
            filho = 2*i + 1;
        }

        if(lista->array[i]->altura < lista->array[filho]->altura){
            swap(i, filho, lista);
            i = filho;
        }else{
            i = tamHeap;
