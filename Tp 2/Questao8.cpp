...
int main(){
    ...
    while(isFim(diretorio) == false){
        ...
        pertence = buscaBinaria(lista, nome);   
        if(pertence == true)printf("SIM\n");
        else printf("NAO\n");
        fgets(nome, 1000, stdin);
    }
...
bool buscaBinaria(Lista *lista, char* nome) {
	...
	while (esquerda <= direita) {
		...
		if (strcmp(lista->array[meio]->nome, nome) == -10){ //são iguais
            //printf("%s %s \n", lista->array[meio]->nome, nome);   
            //printf("%d\n", strcmp(lista->array[meio]->nome, nome));          
            pertence = true;
            esquerda = lista->n;
		} else if (strcmp(lista->array[meio]->nome, nome) < 0){//
                //printf("%s %s", lista->array[meio]->nome, nome); 
                //printf("%d\n", strcmp(lista->array[meio]->nome, nome));          
				esquerda = meio + 1;
        }else{
			direita = meio - 1;
